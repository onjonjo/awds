...
#define max_quality 0xffff
	...
	LinkList(LinkList const &ll):std::vector<LinkQuality>() {
	    *this = ll;
	}
	LinkList::iterator insert(LinkQuality const &lq,AdjList &adjList,const NodeId &me);  // inserts or updates the LinkQuality
    };

    //    typedef std::vector<LinkQuality> LinkList;

    //    typedef std::vector<link_quality_t> QList; 
